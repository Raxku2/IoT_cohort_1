...
| `cd` | Change Directory | Navigates into a different directory or steps back in the file path. | `cd folder_name` | `..`, `/` | Relative/Absolute Path Symbols | `..`: Goes back one hierarchical directory. <br>`/`: Navigates instantly to the root directory. |
| `ls` | List | Lists out files and directories located in the current path. | `ls -ltr` | `-l`, `-t`, `-r`, `-h` | Long list, Time, Reverse, Human-readable | `-l`: Lists items vertically with file sizes and permissions. <br>`-t`: Sorts items by latest modified time. <br>`-r`: Reverses standard sorting order. <br>`-h`: Formats file sizes to be readable. |
...
| `less` | Less | Opens a file in a page-by-page reader layout within the terminal. | `less file.txt` | `/`, `?` | Search Operators | `/word`: Initiates a search from top to bottom. <br>`?word`: Initiates a search from bottom to top. |
...
| `> / >>` | Redirection Operators | Redirects operational payload strings seamlessly into target files. | `ls > files.txt` | `>` / `>>` | Overwrite / Append | `>`: Overwrites a destination file. <br>`>>`: Concatentates output onto an existing file. |
...
| `tar` | Tape Archive | Recursively compresses or extracts comprehensive folders/directories. | `tar -czf x.tar.gz folder` | `-c`, `-x`, `-z`, `-f` | Create, Extract, Gzip, File | `-c`: Provisions a new archive. <br>`-x`: Extracts a pre-existing archive. <br>`-z`: Injects standard gzip compression. <br>`-f`: Denotes the final archive filename. |
...
| Command | Command Name | Description | Example Usage | Flags Used | Flag Name | Flag Description |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `free` | Free Memory | Tracks standard available cache versus utilized RAM memory on the server load. | `free -h` | `-h`, `-t` | Human-readable, Total | `-h`: Displays numbers properly formatted in MB/GB equivalents. <br>`-t`: Aggregates usage parameters to push a summarized "Total" output line. |
...
| `date` | Date | Displays the current system date and time. | `date +%T` | `+%D`, `+%T`, `+%H:%M` | Date Format, Time Format, Custom Format | `+%D`: Shows date only. <br>`+%T`: Shows time only. <br>`+%H:%M`: Formats specific hours and minutes. |
